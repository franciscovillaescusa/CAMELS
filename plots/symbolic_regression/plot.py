...
root = '/mnt/ceph/users/camels/Results/symbolic_regression'
...
ax1.set_xlabel(r'${\rm redshift}$',fontsize=18)
ax1.set_ylabel(r'${\rm error}$',fontsize=18)


f_out='SFRH_SG.pdf'

f1 = '%s/errors6.txt'%root
f2 = '%s/errors9.txt'%root
f3 = '%s/errors8.txt'%root

z1,d,d1,dSFRH1,dlogSFRH1 = np.loadtxt(f1,unpack=True) 
z2,d,d2,dSFRH2,dlogSFRH2 = np.loadtxt(f2,unpack=True) 
z3,d,d3,dSFRH3,dlogSFRH3 = np.loadtxt(f3,unpack=True) 

p1,=ax1.plot(z1,dlogSFRH1,linestyle='-',marker='None',c='r')
p2,=ax1.plot(z2,dlogSFRH2,linestyle='-',marker='None',c='b')
p3,=ax1.plot(z3,dlogSFRH3,linestyle='-',marker='None',c='g')

print(np.mean(dlogSFRH1))
print(np.mean(dlogSFRH2))
print(np.mean(dlogSFRH3))
...
ax1.legend([p1,p2,p3],
           [r"${\rm Eq.\,18}$",
            r"${\rm Eq.\,19}$",
            r"${\rm Eq.\,20}$"],
           loc=0,prop={'size':16},ncol=1,frameon=True)
