...
x_min, x_max = -0.1, 7.1
y_min, y_max = 4e-5, 2.0


f1 = '../../../../Results/neural_nets/params_2_SFRH/SFRH_ML.txt'
f2 = '../../../IllustrisTNG/latin_hypercube_params.txt'

Om,s8,A1,A2,A3,A4 = np.loadtxt(f2, unpack=True)

data = np.loadtxt(f1)
z = data[:,0]
SFRH_pred = data[:,1:151]
SFRH_true = data[:,151:301]
print(np.min(SFRH_pred),np.max(SFRH_pred))
print(np.min(SFRH_true),np.max(SFRH_true))

for i in range(150):

    f_out = 'SFRH_NN_%d.pdf'%i
    fig = figure()
    ...
    p1,=ax1.plot(z,SFRH_pred[:,i],linestyle='-',marker='None',c='k')
    p2,=ax1.plot(z,SFRH_true[:,i],linestyle='--',marker='None',c='k')

    ax1.set_title(r'$\Omega_{\rm m}=%.2f\,\,\sigma_8=%.2f\,\,A_{\rm SN1}=%.2f\,\,A_{\rm SN2}=%.2f\,\,A_{\rm AGN1}=%.2f\,\,A_{\rm AGN2}=%.2f$'%(Om[i],s8[i],A1[i],A3[i],A2[i],A4[i]),size=11.5)
    
    ax1.legend([p1,p2],
               [r"${\rm Neural\,\,network}$",
                r"${\rm IllustrisTNG}$"],
               loc=3,prop={'size':15},ncol=1,frameon=True)
