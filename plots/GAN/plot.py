...
root = '/mnt/ceph/users/camels/Results/GAN'
f_out='GAN_summaries.pdf'

x_min, x_max = 7e-3, 30.0
y_min, y_max = 1e-2, 1e3

#fig=figure()
fig = figure(figsize=(15/1.2,10/1.2))
gs = gridspec.GridSpec(2,2,height_ratios=[5,2],width_ratios=[1,1])
...
ax4.set_ylim([0.82, 1.18])

for ax in [ax1,ax3]:
    ax.set_xlim([1e3, 3e7])

ax1.set_ylim([4e-7, 3e-2])
ax3.set_ylim([0.65, 1.35])

ax2.set_ylabel(r'$P_T(k)\,[{\rm K}^2(h^{-1}{\rm Mpc})^3]$',fontsize=18)
ax4.set_xlabel(r'$k\/[h\/{\rm Mpc}^{-1}]$',fontsize=18)
ax4.set_ylabel(r'${\rm ratio}$',fontsize=18)

ax1.set_ylabel(r'${\rm PDF}$',fontsize=18)
ax3.set_xlabel(r'$T\/[{\rm K}]$',fontsize=18)
ax3.set_ylabel(r'${\rm ratio}$',fontsize=18)

###### power spectrum ######
k1,Pk1,dPk1 = np.loadtxt('%s/test_64/Pk_T_real_64.txt'%root, unpack=True)
k2,Pk2,dPk2 = np.loadtxt('%s/test_64/Pk_T_fake_348_64.txt'%root, unpack=True)

p1=ax2.errorbar(k1,Pk1,yerr=dPk1,lw=1,fmt='o',ms=2,
                elinewidth=1,capsize=5,linestyle='-',c='r') 
p2=ax2.errorbar(k2,Pk2,yerr=dPk2,lw=1,fmt='o',ms=2,
                elinewidth=1,capsize=5,linestyle='-', c='b') 
ax4.plot(k1,Pk1/Pk1,linestyle='-',marker='None',c='r')
ax4.plot(k1,Pk2/Pk1,linestyle='-',marker='None',c='b')


##### PDF #####
T1, PDF1 = np.loadtxt('%s/test_64/PDF_T_real_64.txt'%root, unpack=True)
T2, PDF2 = np.loadtxt('%s/test_64/PDF_T_fake_348_64.txt'%root, unpack=True)

p1,=ax1.plot(T1,PDF1,linestyle='-',marker='None',c='r')
p2,=ax1.plot(T2,PDF2,linestyle='-',marker='None',c='b')
ax3.plot(T1,PDF1/PDF1,linestyle='-',marker='None',c='r')
ax3.plot(T2,PDF2/PDF1,linestyle='-',marker='None',c='b')
