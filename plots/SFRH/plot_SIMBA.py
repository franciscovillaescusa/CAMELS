...
root = '/mnt/ceph/users/camels/Results/SFRH/SIMBA'
f_out='SFRH_SIMBA.pdf'

x_min, x_max = 7e-3, 30.0
y_min, y_max = 6e-4, 0.12

f1 = '%s/mean_SFRH_0.00_10.00_10000.txt'%root
z,SFRH,dSFRH,dSFRH_p,dSFRH_m = np.loadtxt(f1, unpack=True)

z_interp = np.linspace(0.0, 8.0, 100)
SFRH_interp = np.interp(z_interp, z, SFRH)
dSFRH_interp = np.interp(z_interp, z, dSFRH)
dSFRH_p_interp = np.interp(z_interp, z, dSFRH_p)
dSFRH_m_interp = np.interp(z_interp, z, dSFRH_m)
...
ax1.set_ylabel(r'${\rm SFRH}\,[M_\odot{\rm yr}^{-1}{\rm Mpc}^{-3}]$',fontsize=18)


p1,=ax1.plot(z_interp,SFRH_interp,linestyle='-',marker='None',c='b')
ax1.fill_between(z_interp, dSFRH_p_interp, dSFRH_m_interp, color='blue',alpha=0.6)
#p1,=ax1.plot(z,SFRH,linestyle='-',marker='None')

# do a loop over the individual SFRH
for i in range(27):
    fin = '%s/1505_%d/SFRH_0.00_10.00_10000.txt'%(root,i)
    z,SFRH = np.loadtxt(fin, unpack=True)
    z_interp = np.linspace(0.0, 8.0, 100)
    SFRH_interp = np.interp(z_interp, z, SFRH)
    p2,=ax1.plot(z_interp,SFRH_interp,linestyle='-',marker='None',c='k',lw=0.3)
